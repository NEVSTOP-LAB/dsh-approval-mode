...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** @returns the settings API face from the connection service. */
    function settingsApi(ctx) {
      var connection = ctx.get("connection");
      return connection && connection.api ? connection.api : null;
    }

    /** Read the current mode through settings.describe. */
    function readMode(api) {
      return api.settings.describe({}).then((response) => {
        if (!response || !response.result || response.result.ok !== true) return { mode: "ask", revision: undefined };
        var namespaces = response.result.value && response.result.value.namespaces;
        if (!Array.isArray(namespaces)) return { mode: "ask", revision: undefined };
        var view = namespaces.find((candidate) => candidate.ns === NS);
        var mode = view && view.value && view.value.mode === "bypass" ? "bypass" : "ask";
        return { mode, revision: view ? view.revision : undefined };
      });
    }

    /** Write the mode through settings.mutate. */
    function writeMode(api, mode, revision) {
      return api.settings.mutate({
        ns: NS,
        ops: [{ op: "set", path: ["mode"], value: mode }],
        ...(revision === undefined ? {} : { expectedRevision: revision })
      }).then((response) => {
        return !!(response && response.result && response.result.ok === true);
      });
    }
    ...
    function ApprovalModeSelect(props) {
      ...
      react.useEffect(() => {
        ...
        var alive = true;
        readMode(api).then((read) => {
          if (!alive) return;
          setState({ mode: read.mode, ready: true, open: false, revision: read.revision });
        }).catch((err) => {
          console.error("[dsh-approval-mode] read settings failed:", err);
      ...
      return react.createElement("div", { className: "dsh-approval-mode-root" },
        react.createElement("button", {
          ...
          react.createElement("span", { className: "dsh-approval-mode-triggerIcon" },
            svg(bypass ? ICON_BOLT : ICON_SHIELD, { filled: true })),
    ...
    /** Required runtime services: slots (UI seat), connection (settings RPC). */
