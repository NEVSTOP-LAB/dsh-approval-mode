...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var zh = {
      ...
      "card.saving": "正在保存…",
      "card.error": "读写设置失败：无法与 DSH 通信，显示的是最后一次生效的值。",
    ...
    var en = {
      ...
      "card.saving": "Saving…",
      "card.error": "Could not read or write the setting: the DSH host is unreachable, so the last effective value is shown.",
     ...
     * A failed write puts the previous value back; a failed initial read falls
     * back to the schema default ("ask") and raises `failed`, so a surface says
     * the value is unknown instead of presenting a default as the setting.
     * `select` is generation-guarded: the toolbar menu stays operable while a
     * write is in flight, and an older response must not overwrite a newer one.
     */
    var modeStore = {
      state: { mode: "ask", ready: false, saving: false, failed: false },
      listeners: [],
      loading: null,
      generation: 0,
      get: function () {
      ...
      load: function () {
        ...
        if (!canFetch()) {
          modeStore.publish({ mode: "ask", ready: true, saving: false, failed: false });
          return Promise.resolve(modeStore.state);
        }
        modeStore.loading = readMode().then((mode) => {
          modeStore.loading = null;
          modeStore.publish({ mode, ready: true, saving: false, failed: false });
          return modeStore.state;
        }).catch((err) => {
          modeStore.loading = null;
          console.error("[dsh-approval-mode] read failed:", err);
          modeStore.publish({ mode: "ask", ready: true, saving: false, failed: true });
          return modeStore.state;
        });
        return modeStore.loading;
      },
      select: function (mode) {
        var previous = modeStore.state;
        var generation = ++modeStore.generation;
        modeStore.publish({ mode, ready: true, saving: true, failed: false });
        if (!canFetch()) {
          modeStore.publish({ mode, ready: true, saving: false, failed: false });
          return Promise.resolve(true);
        }
        return writeMode(mode).then((ok) => {
          // A newer selection superseded this one: its own write owns the state.
          if (generation !== modeStore.generation) return ok;
          if (!ok) {
            modeStore.publish({ mode: previous.mode, ready: true, saving: false, failed: true });
            return false;
          }
          modeStore.publish({ mode, ready: true, saving: false, failed: false });
          return true;
    ...
    function ApprovalModeSelect(props) {
      ...
      var label = t(effectiveBypass ? "mode.bypass" : "mode.ask");
      var title = fullAccess ? t("title.fullAccess") : t(effectiveBypass ? "title.bypass" : "title.ask");
    ...
    function ApprovalModeSettingsCard(props) {
      ...
      var open = openPair[0];
      var setOpen = openPair[1];
      var bypass = snapshot.mode === "bypass";
      ...
      var option = (value, labelKey) => {
        var selected = snapshot.mode === value;
