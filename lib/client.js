 ...
 * and writes it through the Host control route (same-origin fetch).
 *
 * i18n: registers the "approval-mode" dictionary (zh/en) with the DSH locale
 * service; the slot owner injects the bound `t` via the `locale` registration
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** Required runtime services: slots (UI seat), locale (i18n dictionaries). */
